...
int processifinfo(DSTATE *s, datacache **dc)
{
	...
	if ((*dc)->updated > ifinfo.timestamp) {
		...
		if ((*dc)->updated > (ifinfo.timestamp + 86400)) {
			snprintf(errorstring, 1024, "Interface \"%s\" has previous update date too much in the future, exiting. (%" PRIu64 " / %" PRIu64 ")", (*dc)->interface, (uint64_t)(*dc)->updated, (uint64_t)ifinfo.timestamp);
			printe(PT_Error);
			errorexitdaemon(s, 1);
		}
		return 0;
...
void cleanremovedinterfaces(DSTATE *s)
{
	...
	if (dbifl != NULL) {
		dbifl_iterator = dbifl;
		while (dbifl_iterator != NULL) {
			snprintf(errorstring, 1024, "Interface \"%s\" no longer in database, stopping monitoring.", dbifl_iterator->interface);
			printe(PT_Info);
	...
	if (db_getiflist(&dbifl) > 0 && dbifl != NULL) {
		...
		while (dbifl_iterator != NULL) {
			...
			if (!interface_already_monitored) {
				snprintf(errorstring, 1024, "Interface \"%s\" found from database, starting monitoring.", dbifl_iterator->interface);
				printe(PT_Info);
...
void handleintsignals(DSTATE *s)
{
	switch (intsignal) {
		...
		default:
			snprintf(errorstring, 1024, "Unknown signal %d received, ignoring.", intsignal);
			printe(PT_Info);
			break;
	}
