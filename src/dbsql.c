...
uint64_t db_getinterfacecountbyname(const char *iface)
{
	...
	if (strchr(iface, '+') == NULL) {
		...
		sqlite3_snprintf(512, sql, "select count(*) from interface where name in (%s)", inquery);
		free(inquery);
	}
...
char *db_getinterfaceidin(const char *iface)
{
	...
	sqlite3_snprintf(256, sql, "select group_concat(id) from interface where name in (%s)", inquery);
	free(inquery);
	...
	if (rc == SQLITE_OK) {
		...
		db_errcode = rc;
		snprintf(errorstring, 1024, "Failed to get interface id from database (\"name in\", %d): %s", rc, sqlite3_errmsg(db));
		printe(PT_Error);
	}
...
int db_getdata_range(dbdatalist **dbdata, dbdatalistinfo *listinfo, const char *iface, const char *table, const uint32_t resultlimit, const char *databegin, const char *dataend)
{
	...
	char sql[768], limit[64], dbegin[38], dend[45], *ifaceidin = NULL;
	sqlite3_stmt *sqlstmt;
	time_t timestamp;
	int64_t rowid;
	uint64_t rx, tx;
	...
	if (strlen(databegin)) {
		if (strcmp(databegin, "today") == 0) {
			snprintf(dbegin, 38, " and date >= date('now'%s)", cfg.dbtzmodifier);
		} else {
			snprintf(dbegin, 38, " and date >= '%s'", databegin);
		}
	}
	...
	if (strlen(dataend)) {
		if (strchr(dataend, ':')) {
			snprintf(dend, 45, " and date <= datetime('%s')", dataend);
		} else {
			snprintf(dend, 45, " and date <= datetime('%s 23:59:59')", dataend);
		}
	}
